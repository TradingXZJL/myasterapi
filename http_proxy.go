...
//
//	// 启用连接池并使用默认配置
//	myasterapi.SetConnectionPool(nil)
//
//	// 自定义配置
//	config := myasterapi.ConnectionPoolConfig{
//		Enable:              true,
//		MaxIdleConns:        200,
//		MaxIdleConnsPerHost: 20,
//		IdleConnTimeout:     120 * time.Second,
//	}
//	myasterapi.SetConnectionPool(&config)
//
//	// 禁用连接池（恢复原有每次创建新连接的行为）
//	config := myasterapi.ConnectionPoolConfig{Enable: false}
//	myasterapi.SetConnectionPool(&config)
...
func RequestWithHeaderAndBody(urlStr string, method string, headerMap map[string]string, reqBody []byte, isGzip bool) ([]byte, error) {
	...
	if currentPoolConfig.Enable {
		...
		if UseProxy {
			...
			maxWeight := 0
			switch req.Host {
			case AsterGetRestHostByApiType(SPOT):
				apiType = SPOT
				maxWeight = 2400
			case AsterGetRestHostByApiType(FUTURE):
				apiType = FUTURE
				maxWeight = 2400
			default:
