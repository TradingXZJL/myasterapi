...
var SpotApiMap = map[SpotApi]string{
	...
	SpotSubAccountSubAccountApiIpRestrictionDelete: "/sapi/v1/sub-account/subAccountApi/ipRestriction/ipList", //DELETE接口 删除子账户API Key IP白名单(适用母账户)
	...
	SpotAccount:                 "/api/v1/account",                   //GET接口 账户信息 (USER_DATA)
	...
	//现货订单接口
	SpotOpenOrders: "/api/v1/openOrders", //GET接口 查询当前挂单 (USER_DATA)
	SpotAllOrders:  "/api/v1/allOrders",  //GET接口 查询所有订单 (USER_DATA)

	SpotOrderGet:           "/api/v1/order",               //GET接口 查询订单 (USER_DATA)
	SpotOrderPost:          "/api/v1/order",               //POST 现货下单 (TRADE)
	SpotOrderDelete:        "/api/v1/order",               //DELETE接口 撤销订单 (TRADE)
	SpotOrderCancelReplace: "/api/v1/order/cancelReplace", //POST接口 撤消挂单再下单 (TRADE)
	SpotMyTrades:           "/api/v1/myTrades",            //GET接口 账户成交历史 (USER_DATA)

	//通用接口
	SpotPing:         "/api/v1/ping",         //GET接口 测试连通性
	SpotServerTime:   "/api/v1/time",         //GET接口 获取服务器时间
	SpotExchangeInfo: "/api/v1/exchangeInfo", //GET接口 获取交易规范

	//行情接口
	SpotTickerPrice:      "/api/v1/ticker/price",      //GET接口 获取交易对最新价格
	SpotKlines:           "/api/v1/klines",            //GET接口 获取K线数据
	SpotDepth:            "/api/v1/depth",             //GET接口 获取深度信息
	SpotTrades:           "/api/v1/trades",            //GET接口 近期成交列表
	SpotHistoricalTrades: "/api/v1/historicalTrades",  //GET接口 历史成交记录
	SpotAggTrades:        "/api/v1/aggTrades",         //GET接口 近期成交(归集)
	SpotAvgPrice:         "/api/v1/avgPrice",          //GET接口 当前平均价格
	SpotUiKlines:         "/api/v1/uiKlines",          //GET接口 UIK线数据
	SpotTicker24hr:       "/api/v1/ticker/24hr",       //GET接口 24hr 价格变动情况
	SpotTickerBookTicker: "/api/v1/ticker/bookTicker", //GET接口 当前最优挂单
	SpotTicker:           "/api/v1/ticker",            //GET接口 滚动窗口价格变动统计

	//Ws账户推送相关
	SpotUserDataStreamPost:   "/api/v1/userDataStream", //POST接口 (现货账户)生成 Listen Key (USER_STREAM)
	SpotUserDataStreamPut:    "/api/v1/userDataStream", //PUT接口 (现货账户)延长 Listen Key 有效期 (USER_STREAM)
	SpotUserDataStreamDelete: "/api/v1/userDataStream", //DELETE接口 (现货账户)关闭 Listen Key (USER_STREAM)
